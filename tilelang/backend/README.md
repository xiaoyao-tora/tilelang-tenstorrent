...
| `tilelang/tenstorrent` | `tenstorrent` | Single-Core BF16/FP32 Device Lower for equivalent `T.Tiles`/`T.Parallel`, multi-tile expressions, broadcast, Fill, Typecast, Transpose, rank-2 shared-output GEMM with BF16/FP32 accumulation, Reduction, batch dimensions and bounded control flow. Fragment GEMM has a separate frontend precision/lifetime verifier; its Device lifetime scheduling remains unsupported. Device IR v1 preserves Add; v2 carries general computation; v3 lowers bounded independent pipelines with asynchronous copy completion, pooled DFB capacity, explicit release and payload-budget verification. v4 specializes bounded static Core grids and PipeNet records into per-Core processor functions, explicit point deliveries and verified completion/lifetimes, including multicast, gather, scatter and row/column GEMM distribution. TTL codegen and TTNN execution are not implemented. |
