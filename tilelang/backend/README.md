...
| `tilelang/tenstorrent` | `tenstorrent` | Single-Core BF16/FP32 Device Lower for equivalent `T.Tiles`/`T.Parallel`, multi-tile expressions, broadcast, Fill, Typecast, Transpose, rank-2 shared-output GEMM with BF16/FP32 accumulation, Reduction, batch dimensions and bounded control flow. Device IR v5 schedules persistent fragment GEMM on one Core; v6 combines per-Core accumulators with repeated PipeNet transactions for fixed-source SUMMA. Device IR v1 preserves Add; v2 carries general computation; v3 lowers bounded independent pipelines with asynchronous copy completion, pooled DFB capacity, explicit release and payload-budget verification. v4 specializes bounded static Core grids and PipeNet records into per-Core processor functions, explicit point deliveries and verified completion/lifetimes, including multicast, gather, scatter and row/column GEMM distribution. Device IR v7 adds immutable ordinary fragment values, mixed-scope elementwise expressions, GEMM epilogues, explicit old/new dependencies and exact-dtype materialization on one Core. Source-only TTL emission supports a restricted single-Core subset; FP32/full-K GEMM TTL schedules, multicore TTL codegen and TTNN execution remain unsupported. |
