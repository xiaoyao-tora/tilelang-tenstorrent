...
| `tenstorrent` | Tenstorrent devices. Requires `wormhole_b0` or `blackhole`; Phase 1 can form a verified no-op Device TIR skeleton, while Add lowering and TTL codegen remain unimplemented. |
...
implemented Phase 1 subset is documented in `docs/compiler_internals/tenstorrent_phase1_device_ir.md`. The ten-pass
pipeline can lower a single-Core, no-Pipe, no-DFB semantic no-op into a verified three-slot Device TIR module.
TileOps, compute, dataflow, topology, and multi-Core programs fail explicitly. Compiling still fails at TTL source
